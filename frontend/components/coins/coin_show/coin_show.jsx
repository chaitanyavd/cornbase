...
class CoinShow extends React.Component {
    ...
    render() {
        ...
        let circSupply =
            parseFloat(coin.circulating_supply) > 1000000000
                ? `${(parseFloat(coin.circulating_supply) / 1000000000).toFixed(
                      1
                  )}B ${coin.symbol}`
                : `${(parseFloat(coin.circulating_supply) / 1000000).toFixed(
                      1
                  )}M ${coin.symbol}`;
        let allTimeHigh =
            parseFloat(coin.high) > 0.1
                ? parseFloat(coin.high).toFixed(2)
                : parseFloat(coin.high).toFixed(4);
        let price =
            parseFloat(coin.price) > 0.1
                ? parseFloat(coin.price).toFixed(2)
                : parseFloat(coin.price).toFixed(4);
        let volume =
            parseFloat(coin["1d"].volume) > 1000000000
                ? `$${(parseFloat(coin["1d"].volume) / 1000000000).toFixed(1)}B`
                : `$${(parseFloat(coin["1d"].volume) / 1000000).toFixed(1)}M`;
        ...
        let color = change
            ? change >= 0
                ? "graph-pospercent-change"
                : "graph-negpercent-change"
            : null;
