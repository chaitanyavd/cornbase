...
export const fetchWatchlists = () => dispatch =>
  watchlistApiUtil
    .fetchWatchlists()
    .then(watchlists => dispatch({ type: RECEIVE_WATCHLISTS, watchlists }));

export const createWatchlist = watchlist => dispatch =>
  watchlistApiUtil
    .createWatchlist(watchlist)
    .then(watchlist => dispatch({ type: RECEIVE_WATCHLIST, watchlist }));
...
export const deleteWatchlist = id => dispatch => {
  return watchlistApiUtil.deleteWatchlist(id).then(watchlist => {
    dispatch({ type: REMOVE_WATCHLIST, watchlistId: watchlist.ticker });
  });
};
